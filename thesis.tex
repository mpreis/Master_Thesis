...
\section{Workflow}
We implemented a multistate offline approach to observe all the information. In this context \emph{offline} indicates that there are analyzing steps required before executing the \ac{trace}. More concrete this is about \emph{Preparation}, and \emph{Transformation}.

\begin{description}
  \item[Compilation] It is required to generate a binary of a benchmark. The benchmarks used are compiled with GCC 4.8.5 on Ubuntu 16.04 for AMD Opteron\texttrademark Processor 6376 with x86\_64 Architecture.
  \item[Preparation] Next is to obtain a \ac{trace} of load and store instruction on addresses. We use the Valgrind\cite{Valgrind} tool called \emph{Lackey} obtain all memory accesses of an executable. This represents the \ac{trace} of the benchmark which will be transformed.
  \item[Transformation \& Analysis] After the \ac{trace} has been obtained two steps are handled simultaneously, (1) transforming the \ac{trace} from an address-based \ac{trace} to a variable-based \ac{trace} and (2) observing the metrics explained in \Cref{sec:metrics}.
  \item[Execution] Finally the transformed \ac{trace} can be executed to figure out its performance. This procedure requires to apply a cache model. In this work there are several cache models available which are presented by \Cref{sec:cache-models}. During execution the memory accesses are counted which are finally used to calculate the performance.
\end{description}

\section{Preparation: Valgrind} % Valgrind

\section{Compilation: Benchmarks}
